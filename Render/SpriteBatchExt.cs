...
namespace zxc {
	...
	{
		private static Rectangle _tempRect = new Rectangle(0, 0, 0, 0);
		...
		public static void DrawLineAngle(this SpriteBatch spriteBatch, Vector2 start, float angle, float length,
		                                 Color color)
		{
			spriteBatch.Draw(Core.BasicTexture, start, null, color,
		                            ...
		                            Color color)
		{
			...
			spriteBatch.Draw(Core.BasicTexture, _tempRect, null,
				color);
		}
		...
		{
			_tempRect = rect;
			spriteBatch.Draw(Core.BasicTexture, rect, null, color);
		}
		...
		public static void DrawPixel(this SpriteBatch spriteBatch, Vector2 position, Color color, int size = 1)
		{
			...
			if (size != 1)
			{
				rect.X -= (int)(size * 0.5f);
				rect.Y -= (int)(size * 0.5f);
			}
